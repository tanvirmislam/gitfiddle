...
class CommandHandler {
    ...
    getValidCommands (cmd) {
      const found = cmd.match(this._regex)
    ...
    process (cmdObject, tree, history) {
      ...
      const command = cmdObject.command
    ...
    commit (operationType, tree, cmdObject, history) {
      switch (operationType) {
        case 'do': {
          const newNode = new Node(tree.nextId, tree.nodeDiameter)
    ...
    merge (operationType, mergeWithBranchName, tree, cmdObject, history) {
      switch (operationType) {
        case 'do': {
          ...
          const mergeWithNode = tree.getNodeFromBranchName(mergeWithBranchName)
          ...
          if (mergeWithNode !== undefined) {
            ...
            cmdObject.undoInfo.mergedWithBranchName = mergeWithBranchName
            ...
            tree.switchBranch(mergeWithBranchName, mergeWithNode, tree.currentBranchNode)
          } else {
            return false
          }
        ...
        case 'undo': {
          // History will be popped at commit's undo
          const currentNode = tree.currentBranchNode
          const mergedWithBranchName = history[history.length - 1].undoInfo.mergedWithBranchName
          const mergedWithNode = tree.getNodeFromId(history[history.length - 1].undoInfo.mergedWithNodeId)

          tree.switchBranch(mergedWithBranchName, tree.currentBranchNode, mergedWithNode)
    ...
    rebase (operationType, branchName, tree, cmdObject, history) {
      switch (operationType) {
        case 'do': {
          ...
          const currentNode = tree.currentBranchNode
          const rebaseNode = tree.getNodeFromBranchName(branchName)
          ...
          const lcaInfo = tree.getLCAInfo(currentNode, rebaseNode)
          const count = tree.getNodeCountTillAncestor(currentNode, lcaInfo.lcaNode)
          const branchSpecificPath = tree.getBranchSpecificPath(tree.currentBranchName)

          const createNodesInfo = []
          const removeNodesInfo = []
        ...
        case 'undo': {
          ...
          if (popped.undoInfo.rebaseUndoType === 1) {
            const fromNode = tree.getNodeFromId(popped.undoInfo.switchBranchFromNodeId)
            const toNode = tree.getNodeFromId(popped.undoInfo.switchBranchToNodeId)
            tree.switchBranch(tree.currentBranchName, fromNode, toNode)
          } else {
            for (let i = 0; i < popped.undoInfo.createNodesInfo.length; ++i) {
              const newNode = new Node(popped.undoInfo.createNodesInfo[i].id, popped.undoInfo.createNodesInfo[i].diameter)

              for (let j = 0; j < popped.undoInfo.createNodesInfo[i].parentIds.length; ++j) {
                const parentNode = tree.getNodeFromId(popped.undoInfo.createNodesInfo[i].parentIds[j])
            ...
            for (let i = 0; i < popped.undoInfo.removeNodesInfo.length; ++i) {
              const node = tree.getNodeFromId(popped.undoInfo.removeNodesInfo[i].id)
              tree.remove(node)
            }
          }
    ...
    push (operationType, tree, cmdObject, history) {
      switch (operationType) {
        case 'do': {
          const pathsToRoot = tree.getPathsToNode(tree.currentBranchNode, tree.root)
          const pushedNodeIds = []
