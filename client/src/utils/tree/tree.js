...
class Tree {
    ...
    updateTreeInfo () {
      ...
      nextLevel.add(this._root)
      this._info.maxYCoord = this._root.y

      while (currentLevel.length !== 0 || nextLevel.size !== 0) {
        if (currentLevel.length === 0) {
          this._info.totalNodes += nextLevel.size
          this._info.width = Math.max(this._info.width, nextLevel.size)
          this._info.height += 1

          this._info.levelOrder.push(Array.from(nextLevel))
          visited = new Set([...visited, ...nextLevel])

          currentLevel.push(...Array.from(nextLevel))
          nextLevel = new Set()
        } else {
          ...
          node.children.forEach((childNode) => {
            if (!visited.has(childNode)) {
              ...
              this._info.maxYCoord = Math.max(this._info.maxYCoord, childNode.y)
            }
          })
        }
      }
